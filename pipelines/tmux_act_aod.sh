...
# Usage:
#
#   ./tmux_act.sh  <environment_setup_script> <act_home> <darknet_root> <vis_directory> <algorithm_time>
#
...
tmux send-keys -t $SESSION:0 "pipeline_runner --pipe ${SCRIPT_DIR}/image_sender.pipe --set exp:experiment_file_name=etc/input_stream.yml --set zmq_timg:port 5760" C-m
...
tmux send-keys -t 1 "pipeline_runner --pipe ${SCRIPT_DIR}/act_aod_receiver.pipe --set act_process:exp=${ACT_HOME}/virat-act-detectory-scripts/rgb_eval_1b.yml --set --set zmq::port=5760 --set img_writer::file_name_template=${VIS_DIRECTORY}/image%06d.jpg" C-m

...
tmux send-keys -t 0 "pipeline_runner --pipe0${SCRIPT_DIR}/json_writer_receiver.pipe --set connect_host 192.168.1.142" C-m
...
tmux send-keys -t 1 "pipeline_runner --pipe ${SCRIPT_DIR}/visualizer_receiver.pipe --set conect_host 192.168.1.142" C-m

sleep 1
echo "$(date) Preparing Swimlane Visualizer..."
#tmux send-keys -t 3 "cd ${SCRIPT_DIR}" C-m
tmux send-keys -t 2 "source ${START_SCRIPT}" C-m
tmux send-keys -t 2 "pipeline_runner --pipe ${SCRIPT_DIR}/swimlane_receiver.pipe --set connect_host 192.168.1.142" C-m
